...
const YAxis = ({ xScale, yScale, yTicksNormalized, axisSide, labelPosition, labelOffset, labelColor, lineWidth, lineColor, font, labelRenderer, formatYLabel = (label) => String(label), linePathEffect, chartBounds, }) => {
    var _a;
    ...
    const yAxisNodes = yTicksNormalized.map((tick) => {
        ...
        {font || labelRenderer
                ? canFitLabelContent && (<>
                {labelRenderer ? (labelRenderer.render({
                        text: contentY,
                        color: labelColor,
                        x: labelX,
                        y: labelY,
                        width: labelWidth,
                        height: labelHeight,
                    })) : (<react_native_skia_1.Text color={labelColor} text={contentY} font={font !== null && font !== void 0 ? font : null} y={labelY} x={labelX}/>)}
              </>)
                : null}
