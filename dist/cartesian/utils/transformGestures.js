...
const panTransformGesture = (state, _config = {}) => {
    ...
    const pan = react_native_gesture_handler_1.Gesture.Pan()
        ...
        .onChange((e) => {
        var _a, _b, _c, _d;
        const newMatrix = (0, react_native_skia_1.multiply4)((0, react_native_skia_1.translate)(panX ? e.changeX : 0, panY ? e.changeY : 0, 0), state.matrix.value);
        ...
        if (config.canvasWidth !== undefined && panX) {
            const scaleX = (_a = newMatrix[0]) !== null && _a !== void 0 ? _a : 1;
            const minTx = config.canvasWidth * (1 - scaleX);
            newMatrix[3] = Math.min(Math.max(((_b = newMatrix[3]) !== null && _b !== void 0 ? _b : 0), minTx), 0);
        }
        if (config.canvasHeight !== undefined && panY) {
            const scaleY = (_c = newMatrix[5]) !== null && _c !== void 0 ? _c : 1;
            const minTy = config.canvasHeight * (1 - scaleY);
            newMatrix[7] = Math.min(Math.max(((_d = newMatrix[7]) !== null && _d !== void 0 ? _d : 0), minTy), 0);
        }
